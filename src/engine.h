...
public:
	...
	bool isGoal( Node *node, bool progressive_eval ) override{
		...
		auto vps = p->run(_gpp, plans, true, progressive_eval );
	...
	vector< Node* > expandNode( Node *node, bool incremental_eval ) override{
        ...
        if( IFS_ALLOWED ) {
            ...
            vps = p->run(_gpp, plans, true, incremental_eval);
	...
	Node* solve(bool progressive) override{
        ...
        for( int pl = 0; pl < (int) ins_names.size(); pl++ ) {
            ...
            auto vps = root_program->run(_gpp, plans, true, progressive);
            freeProgramStates(vps);
		...
		while( !isEmpty() ){		
			...
			for( int i = 0; i < int( childs.size() ); i++ ){
                ...
                auto vps = childs[ i ]->getProgram()->run(_gpp, plans,true, progressive);
                freeProgramStates(vps);
				if( vps.empty() ) {
                    _deadends++;
                    delete childs[ i ];
				...
				if( isGoal( childs[ i ], progressive ) ){
                    ...
                    if( not DEFERRED_EVALUATION and REEVALUATE_OPEN_QUEUE ){
                        ...
                        while( !_open.empty() ) {
                            ...
                            vector<vector<string>> plans;
                            auto vps2 = aux_current->getProgram()->run(_gpp, plans,true,progressive);

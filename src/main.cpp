...
int main(  int argc, const char* argv[] ){
	if( argc < 5 ){
		...
		cout << "[INFO] Execute: ./main.bin <BFS|PGP> <program_lines> <problem_folder/> <outfile> <list of heuristic names>" << endl;
		cout << "[INFO] Example: ./main.bin PGP 5 domains/synthesis/visitall/ landmarks solution h6" << endl;
    ...
    vector< string > heuristic_names( argc - 5 );
    string heuristics;
    bool preprocess_landmarks = false;
    for( int i = 5; i < argc; i++ ) {
        heuristic_names[ i-5 ] = string(argv[i]);
        heuristics += "_" + heuristic_names[i-5];
        if( heuristic_names[ i-5 ] == "landmarks" or heuristic_names[i-5] == "normlandmarks")
            preprocess_landmarks = true;
    }
    if( argc == 5 ){ // default heuristic
    ...
    if( preprocess_landmarks ) {
        for (int id = 0; id < gpp->getNumInstances(); id++) {
            ...
            ofstream ofs_prog("instance_" + to_string(id) + ".lg" );
	...
	if( res != nullptr ){
        ...
        */
		...
		cout << output;	
		cout << "[INFO] Program file: " << outfile << endl;	
	}	
	else{
	...
	cout << "[INFO] Experiments file: " << "output" << endl;	

...
public:
	...
	void addObject( StateDescriptor *sd, const string &name, const string &obj_type ){
	    ...
	    typed_address_to_obj[ make_pair(obj_type, num_of_objects) ] = name;
	...
	string getObjectName(const string& obj_type, int address) const{
		auto it =  typed_address_to_obj.find(make_pair(obj_type,address));
		assert( it != typed_address_to_obj.end());
		return it->second;
	}
...
private:
	...
	map<pair<string,int>, string> typed_address_to_obj;
};

...
int main(  int argc, const char* argv[] ){
	...
	for( int l = 0; l < program_lines; l++ ){
		if( program_instructions[l] == "empty" ) continue;
	...
	vector< ProgramState* > vps = p->run( gpp, plans, infinite_detection, false );
	...
	if( int( vps.size() ) > 0 ){
		for( int l = 0; l < int( vps.size() ); l++ ){
			...
			#endif
			ofstream ofs("plan."+to_string(l+1));
			for(const auto& action : plans[l])
				ofs << action << "\n";
			ofs.close();
